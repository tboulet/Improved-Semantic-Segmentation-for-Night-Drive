...
class cyclegan(object):
    ...
    def process_dataset(self, args):
        ...
        self.saver.restore(self.sess, os.path.join(args.checkpoint_path))
        print(" [*] Load SUCCESS")
        out_var,refine_var, in_var,rec_var,cycle_var = (self.testB,self.refine_testB, self.test_A,self.rec_testA,self.rec_cycle_A) if args.which_direction == 'AtoB' else (
            self.testA,self.refine_testA, self.test_B,self.rec_testB,self.rec_cycle_B)
        for sample_file in tqdm(sample_files):
            ...
            if not os.path.exists(refine_fake_folder):
                os.makedirs(refine_fake_folder)

            save_images(fake_img, [1, 1], os.path.join(fake_img_folder, os.path.basename(sample_file)))
            save_images(refine_fake, [1, 1], os.path.join(refine_fake_folder, os.path.basename(sample_file)))

...
def softmax_weighted_cce(
    ...
    """Compute the weight categorical crossentropy
    ...
    _predictions = tf.reshape(
        predictions_logits, shape=(-1, predictions_logits.shape[-1])
    )
    _weights = tf.expand_dims(weights, axis=0)

    loss = -tf.math.reduce_sum(
        _targets * tf.math.log_softmax(_predictions) * _weights, axis=-1
    )
